...
/**
 * Resolves ntfy topics from (in order): AGENT_PING_OUTBOUND_TOPIC /
 * AGENT_PING_REPLY_TOPIC env vars, or a config.json in the current
 * directory (or AGENT_PING_CONFIG). Throws if topics are missing,
 * placeholder-looking, too short, or identical to each other.
 */
function getConfig() {
  ...
  if (envOut || envReply) {
    ...
    if (!configPath) {
      throw new Error(
        'No config found. Set AGENT_PING_OUTBOUND_TOPIC and AGENT_PING_REPLY_TOPIC env vars, ' +
 ...
 * pending decision instead of overwriting it. Returns
 * { decisionId, queued, waitsFor? } or { decisionId, queued: false, expiresAt }.
 */
async function askDecision({ taskName, question, options, priority = 'high', expiryMinutes = 720 }) {
  ...
  try {
    ...
    await withLock(async () => {
      ...
      try {
        await promoteWaiting(state, config.outboundTopic, config.replyTopic, priorityNum);
      } catch (_) {
