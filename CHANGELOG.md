...
- Added `.markdownlint.json` (disables MD013 line-length) so long-form README lines cannot fail CI spuriously.
- Raised the npm `engines` floor to Node >= 18 to match the stable `node:test` runner.
- Restored waiting-decision promotion and retry-safe send failure handling.
