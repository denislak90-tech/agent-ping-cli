# AGENTS.md - agent call conventions for agent-ping

Read this before wiring an agent to this project. Two equivalent implementations exist - pick whichever fits the host OS. Both read/write the same state format, so you can mix them (for example, ask from one and watch from the other).
...
- Never invent topics, IDs, or state paths - read them from command stdout / config.
...
- exit 0 -> `DECISION_ID=<id>` pending, notification sent
- exit 2 -> `DECISION_ID=<id>` queued as waiting (another decision pending); do other work, listen later
- exit 3 -> lock busy; retry shortly. Never bypass the queue.

Listen (foreground, long timeout):

- exit 0 -> `RESPONSE_RECEIVED`, `DECISION_ID=`, `OPTION=`, `MESSAGE_ID=` - resume that exact task
- exit 1 -> `TIMEOUT_NO_RESPONSE` - keep state, notify human through normal channels

## Rules

- Max 3 options; keep questions under ~200 chars, ASCII, no secrets/PII.
- One pending decision at a time across processes - concurrent sends queue automatically.
